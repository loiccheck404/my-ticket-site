...
function simulatePayment(walletAddress, expectedAmount, cryptocurrency) {
    ...
    if (checkCount < 3) {
        return {
            ...
            isComplete: false,
            confirmations: 0,
            needsMoreConfirmations: walletConfig_1.REQUIRED_CONFIRMATIONS[cryptocurrency],
        };
    }
    // Simulate: Payment found but needs confirmations
    const confirmations = Math.min(checkCount - 2, walletConfig_1.REQUIRED_CONFIRMATIONS[cryptocurrency]);
    const requiredConfs = walletConfig_1.REQUIRED_CONFIRMATIONS[cryptocurrency];
 ...
 */
async function checkBitcoinPayment(walletAddress, expectedAmount, sinceTimestamp) {
    try {
        // BlockCypher API for Bitcoin testnet
        const response = await axios_1.default.get(`https://api.blockcypher.com/v1/btc/test3/addrs/${walletAddress}/full?limit=10`);
        ...
        for (const tx of transactions) {
            ...
            if (Math.abs(receivedAmount - expectedAmount) <= tolerance) {
                ...
                const requiredConfs = walletConfig_1.REQUIRED_CONFIRMATIONS.BTC;
                return {
                    found: true,
                    transaction: {
                        txHash: tx.hash,
        ...
        return {
            ...
            isComplete: false,
            confirmations: 0,
            needsMoreConfirmations: walletConfig_1.REQUIRED_CONFIRMATIONS.BTC,
        };
    }
    catch (error) {
 ...
 */
async function checkEthereumPayment(walletAddress, expectedAmount, cryptocurrency, sinceTimestamp) {
    try {
        ...
        return {
            ...
            isComplete: false,
            confirmations: 0,
            needsMoreConfirmations: walletConfig_1.REQUIRED_CONFIRMATIONS.ETH,
        };
    }
    catch (error) {
 ...
 */
async function checkPaymentStatus(cryptocurrency, expectedAmount, createdAt) {
    ...
    if (cryptocurrency === "BTC") {
        ...
        return await checkEthereumPayment(walletAddress, expectedAmount, cryptocurrency, createdAt);
    }
}
/**
 ...
 */
function shouldContinueChecking(expiresAt, paymentResult) {
    const now = new Date();
 ...
 */
function getPaymentStatusMessage(result) {
    if (!result.found) {
