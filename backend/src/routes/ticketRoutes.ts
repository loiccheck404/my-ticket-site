...
import { PDFTicketGenerator } from "../services/pdfTicketGenerator";
...
router.get("/download/:orderId", async (req: Request, res: Response) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="ticket-${orderId}.pdf"`
    );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="ticket-${orderId}-${index + 1}.pdf"`
      );
